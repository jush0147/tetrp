...
test('update waits for a pending atomic replacement, restores position and never loops',async({page})=>{
  ...
  const server=createServer(async(req,res)=>{
    ...
    res.setHeader('Cache-Control','no-store');res.setHeader('Content-Type',name.endsWith('.jpg')?'image/jpeg':name.endsWith('.wasm')?'application/wasm':name.endsWith('.js')?'text/javascript':name.endsWith('.css')?'text/css':name.endsWith('.png')?'image/png':name.endsWith('.webmanifest')?'application/manifest+json':'text/html');res.end(data);
