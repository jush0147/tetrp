...
function stop(){if(playing)clock.pause(performance.now());playing=false;cancelAnimationFrame(raf);$('play').textContent='播放';$('play').setAttribute('aria-pressed','false');}
...
function load(file){
  ...
  $('welcome').hidden=true;$('workspace').hidden=false;$('selectors').hidden=true;$('filename').textContent=file.name;busy('正在本機讀取 replay…');
...
function conformance(prefix,c){
  ...
  el('status-text').textContent=c.first?'終局比對有差異':c.fields.length?'未發現已知差異':'缺少可比對紀錄';
...
function renderLane(prefix,view,initial){
  ...
  el('lane-display').hidden=Boolean(view.error);el('lane-error').hidden=!view.error;el('conformance').hidden=Boolean(view.error);
...
function renderRound(m,initial){
  ...
  $('placement-input').max=String(total);$('placement-input').value=String(desired);$('placement-input').disabled=!state;
  document.querySelector('#placement-form button').disabled=!state;$('placement-help').textContent=state?'':'請選擇支援的玩家';$('total').textContent=state?`/ ${total}`:'/ —';
  $('frame-input').value=String(roundFrame);$('previous').disabled=!state||desired===0;$('next-placement').disabled=!state||desired>=total;$('play').disabled=!available;
...
$('play').addEventListener('click',()=>{if(playing){stop();return;}if(!available)return;
  ...
  playing=true;clock.start(at,performance.now());$('play').textContent='暫停';$('play').setAttribute('aria-pressed','true');raf=requestAnimationFrame(tick);
});
