...
BASE_ATTACK = {(0, 'none'): 0, (0, 'mini'): 0, (0, 'full'): 0, (1, 'none'): 0, (1, 'mini'): 0, (1, 'full'): 2, (2, 'none'): 1, (2, 'mini'): 1, (2, 'full'): 4, (3, 'none'): 2, (3, 'mini'): 2, (3, 'full'): 6, (4, 'none'): 4, (4, 'mini'): 4, (4, 'full'): 10, (5, 'none'): 5, (5, 'full'): 12}
...
class TetraLeagueV19:
    ...
    def _send(self, amount: int, target: Optional[str]=None) -> Optional[dict]:
        ...
        event = {'type': 'garbage', 'from': self.player_id, 'to': target, 'iid': iid, 'ackiid': self.s.incoming_ack.get(target, 0), 'amt': amount}
    ...
    def receive_interaction(self, event: dict) -> Optional[int]:
        ...
        self.s.next_cid += 1
        cid = self.s.next_cid
        self.s.pending.append(Packet(cid=cid, sender=sender, iid=iid, ackiid=ackiid, amt=amount, active=False))
        return cid
    ...
    def inject_test_pending(self, amount: int, *, active: bool=True, hardened: bool=False, sender: str='TEST') -> int:
        self.s.next_cid += 1
        cid = self.s.next_cid
        self.s.pending.append(Packet(cid=cid, sender=sender, iid=0, ackiid=0, amt=amount, active=active, hardened=hardened, active_frame=self.s.frame if active else None))
        return cid
    ...
    def fight_lines(self, attack: int, *, target: Optional[str]=None) -> dict:
        ...
        defense = 0
        if self.s.piecesplaced <= self.OPENER_PHASE and self.pending_count() >= self.s.cumulative_sent:
            defense += original
        ...
        return {'generated': original, 'cancelled': cancelled, 'sent': 0 if sent_event is None else sent_event['amt'], 'unused_defense_budget': defense}
    ...
    def _surge_packets_if_breaking(self, lines: int, b2b_contribution: int) -> List[int]:
        ...
        if self.s.btb <= self.B2B_CHARGE_AT:
            self.s.btb = 0
            return []
        surge = floor((self.s.btb - self.B2B_CHARGE_AT + self.B2B_CHARGE_BASE) * self.s.garbage_multiplier)
        a = js_round_nonnegative(surge / 3)
        packets = [a, a, surge - 2 * a]
        self.s.btb = 0
        return [x for x in packets if x]

    def _normal_clear_attack(self, lines: int, spin: Spin, *, all_clear: bool, garbage_rows_cleared: int, b2b_contribution: int) -> int:
        ...
        attack = floor(raw * self.s.garbage_multiplier)
    ...
    def lock(self, lines: int=0, spin: Spin='none', *, all_clear: bool=False, garbage_rows_cleared: int=0) -> dict:
        ...
        normal_attack = self._normal_clear_attack(lines, spin, all_clear=all_clear, garbage_rows_cleared=garbage_rows_cleared, b2b_contribution=b2b_contribution)
        normal_result = self.fight_lines(normal_attack) if lines or normal_attack else {'generated': 0, 'cancelled': 0, 'sent': 0, 'unused_defense_budget': 0}
        ...
        return {'frame': self.s.frame, 'piece': self.s.piecesplaced, 'lines': lines, 'spin': spin, 'combo_internal': self.s.combo, 'combo_display': max(0, self.s.combo - 1), 'btb_raw': self.s.btb, 'garbage_multiplier': self.s.garbage_multiplier, 'surge': surge_results, 'normal': normal_result, 'all_clear': all_clear_result, 'blocked_tank': blocked_tank, 'tanked': tanked, 'pending_after': self.pending_count()}
