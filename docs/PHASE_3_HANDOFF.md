...
Initial screen now uses the empty multiplayer viewer shell: one visible board in portrait, two in landscape, no invented active piece or queue, counters/players shown as —, and replay controls disabled. A centered Open replay button launches the local file chooser. Successful file selection starts normal playback at Round 1; solo files switch to the single-player layout. No Engine instance is created for the empty presentation. Refresh/offline reopening returns to this shell without persisting replay files.
