...
The service worker caches only eight explicit public build assets, never replay files. Its scope/start URL are relative to the GitHub Pages subdirectory. Original icons are generated from source at build time; no binary assets or private samples enter Git. Content-hashed cache versions activate immediately after every asset is downloaded with HTTP cache bypass. Opening, returning to the app or reconnecting checks for updates. Activation claims and reloads scoped app windows, including legacy pages without an update listener. Loaded replay data is not persisted: reselect the file after updates. Offline or failed downloads keep the previous complete version.
